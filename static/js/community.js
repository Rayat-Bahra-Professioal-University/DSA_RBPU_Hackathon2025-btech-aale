...
let forumPosts = JSON.parse(localStorage.getItem("forumPosts")) || [
  {
    ...
    category: "Sanitation",
    content:
      "The garbage collection in Model Town has been irregular for the past week. Bins are overflowing and attracting stray animals. Has anyone else faced this issue?",
    comments: 14,
    views: 87,
    likes: 23,
...
let volunteerOpportunities = JSON.parse(
...
) || [
  {
    ...
    description:
      "Help manage traffic during peak hours at busy intersections and assist pedestrians with safe crossing.",
...
function loadWhatsAppGroups() {
  ...
  container.innerHTML = whatsappGroups
    .map(
      (group) => `
...
function loadVolunteerOpportunities() {
  ...
  container.innerHTML = volunteerOpportunities
    .map(
      (opportunity) => `
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", openForumModal);
  ...
  document
    ...
    .addEventListener("submit", createWhatsAppGroup);
  document
    .getElementById("forumForm")
    .addEventListener("submit", createForumPost);
  document
    .getElementById("volunteerForm")
    .addEventListener("submit", submitVolunteerApplication);
}
...
function joinWhatsAppGroup(groupName, groupLink) {
  ...
  if (
    confirm(
      `You're about to join the "${group.name}" WhatsApp community. Continue?`
    )
  ) {
...
function applyForVolunteer(position) {
  ...
  document.querySelector(
    ".modal-header h2"
  ).textContent = `Apply for: ${position}`;
}
...
function submitVolunteerApplication(e) {
  ...
  const applications =
    JSON.parse(localStorage.getItem("volunteerApplications")) || [];
  applications.push({
    ...formData,
    id: "app-" + Date.now(),
  ...
  alert(
    `Thank you ${formData.name}! We have received your application for "${formData.position}". We will contact you at ${formData.email} soon.`
  );
...
function updateCommunityStats() {
  ...
  document.getElementById("totalMembers").textContent =
    whatsappGroups.reduce((total, group) => {
  ...
  document.getElementById("totalPosts").textContent = forumPosts.reduce(
    (total, post) => {
